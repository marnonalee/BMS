...
if($resident_id && $first_name && $last_name){
    $old = $conn->query("SELECT first_name, last_name, household_id FROM residents WHERE resident_id = $resident_id")->fetch_assoc();
    ...
    if($stmt->execute()){
        ...
        $head = $conn->query("SELECT first_name, last_name FROM residents WHERE household_id = {$old['household_id']} AND is_family_head = 1 AND is_archived = 0")->fetch_assoc();
        $head_name = $head ? $head['first_name'].' '.$head['last_name'] : 'Unknown Head';

        $description = "Changed resident name from ".$old['first_name']." ".$old['last_name']." to $first_name $last_name in the family of $head_name";
