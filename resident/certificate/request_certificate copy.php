...
$residentQuery = $conn->prepare("
    SELECT resident_id, first_name, middle_name, last_name, resident_address, birthdate, birth_place, civil_status, sex, voter_status, profession_occupation
    FROM residents
    WHERE user_id = ?
");
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    $template_id = isset($_POST['template_id']) ? intval($_POST['template_id']) : $template_id;
}
...
if ($template_id > 0) {
    ...
    $result = $stmt->get_result();
    if ($result->num_rows > 0) {
        $certificate_name = $result->fetch_assoc()['template_name'];
    } else {
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && !$disableForm) {
 ...
 if (empty($error)) {
    if ($editRequest) {
        ...
        $stmt->bind_param("issdsssii", $template_id, $purpose, $valid_id, $earnings_per_month, $child_fullname, $child_birthdate, $child_birthplace, $editId, $resident_id);
        ...
        $stmt->bind_param("iissdsss", $resident_id, $template_id, $purpose, $valid_id, $earnings_per_month, $child_fullname, $child_birthdate, $child_birthplace);
        if ($stmt->execute()) {
    ...
    <div class="w-full max-w-6xl">
        ...
        <div class="bg-white p-10 rounded-2xl shadow-lg">
            <?php if(!empty($success)): ?>
                <p class="text-green-600 font-medium mb-6"><?= htmlspecialchars($success) ?></p>
            <?php elseif(!empty($error)): ?>
                <p class="text-red-600 font-medium mb-6"><?= htmlspecialchars($error) ?></p>
            <?php endif; ?>
            ...
            <form class="space-y-6" method="POST" enctype="multipart/form-data">
                ...
                <!-- Age, Civil Status, Sex, Voter Status -->
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                ...
                <div id="purposeDiv">
                    ...
                    <textarea name="purpose" class="w-full border p-4 rounded-lg resize-none" rows="3"><?= $editRequest['purpose'] ?? '' ?></textarea>
                </div>

                <!-- Upload ID -->
                <div>
                    <label class="block font-medium mb-2">Upload Valid ID</label>
                    <input type="file" name="valid_id" accept=".jpg,.jpeg,.png" <?= $editRequest ? '' : 'required' ?> class="w-full border p-4 rounded-lg">
                </div>
                ...
                <div id="professionDiv" class="<?= ($certificate_name === 'Certificate of Attestation') ? '' : 'hidden' ?>">
                    ...
                    <input 
                            ...
                            value="<?= $editRequest['profession_occupation'] ?? $resident['profession_occupation'] ?? '' ?>" 
                ...
                <div id="earningsDiv" class="<?= ($certificate_name === 'Certificate of Attestation') ? '' : 'hidden' ?>">
                    ...
                    <input type="number" name="earnings_per_month" min="0" step="any" class="w-full border p-4 rounded-lg">
                </div>

                <!-- Child / Guardian Section -->
                <div id="guardianSection" class="<?= ($certificate_name === 'Certificate of Guardianship' || $editRequest) ? '' : 'hidden' ?> mt-6">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="child_search" placeholder="Type child name..." autocomplete="off" class="w-full border p-4 rounded-lg outline-none">
                    ...
                    <div id="child_manual" class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                        <div>
                            ...
                            <input type="text" name="child_fullname" id="child_fullname" class="w-full border p-4 rounded-lg">
                        </div>
                        <div>
                            <label class="block mb-2 font-medium">Child Age</label>
                            <input type="text" id="child_age" class="w-full border p-4 rounded-lg">
                        </div>
                        <div>
                            <label class="block mb-2 font-medium">Birthdate</label>
                            <input type="date" name="child_birthdate" id="child_birthdate" class="w-full border p-4 rounded-lg">
                        </div>
                        <div>
                            <label class="block mb-2 font-medium">Birthplace</label>
                            <input type="text" name="child_birthplace" id="child_birthplace" class="w-full border p-4 rounded-lg">
                        </div>
                    </div>
                </div>
        ...
        .then(data => {
            ...
            data.forEach(child => {
                ...
                item.addEventListener("click", () => {
                    ...
                    document.getElementById("child_card").classList.add("hidden");
