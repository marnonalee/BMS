...
$checkQuery = $conn->prepare("SELECT status FROM barangay_id_requests WHERE id=? AND resident_id=?");
...
$updateQuery = $conn->prepare("UPDATE barangay_id_requests SET status='Cancelled' WHERE id=? AND resident_id=?");
